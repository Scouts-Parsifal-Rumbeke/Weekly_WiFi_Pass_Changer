EMUI.wifiObjCtrl = EMUI.ObjController.extend({
  ...
  checkPWStrength: function (passValue) {
    ...
    if (sPWLength < 6 || sPWModes === 1) {
      ...
      if (
        parseFloat((repeatPass / parseInt(sPWLength, 10)).toString()) > 0.667
      ) {
        ret = this.MACRO_PASSWORD_LOW;
      } else {
      ...
      if (
        parseFloat((repeatPass / parseInt(sPWLength, 10)).toString()) > 0.334 &&
        parseFloat((repeatPass / parseInt(sPWLength, 10)).toString()) <= 0.667
      ) {
        ret = this.MACRO_PASSWORD_MID;
      } else if (
  ...
  checkVisibleChar: function (str) {
    ...
    for (i = 0; i < str.length; i++) {
      ...
      if (
        numCharStr > this.MACRO_SUPPORT_CHAR_MAX ||
        numCharStr < this.MACRO_SUPPORT_CHAR_MIN
      ) {
        return false;
      }
    }
  ...
  checkWifiKey: function (idprefix, specialcharenable) {
    ...
    if (
      authMethod === this.WIFIAUTHMODE_WPA2_PSK ||
      ...
      authMethod === this.WIFIAUTHMODE_802 ||
      ...
      if (!isValidIpAddress(radiusIPAddress, true)) {
        showErrorMsg(
          idprefix + "_radius_address_div",
          "dialup_hint_ip_address_empty"
        );
        return false;
      }
      if (
        !VALIDATION.isVaildSpecialPort(
          radiusPort,
          idprefix + "_radius_port_div"
        )
      ) {
        return false;
      }
  ...
  showPwdElement: function (authMode, selectID) {
    ...
    if (typeof authMode === "undefined") {
      ...
      if (this.WIFIAUTHMODE_OPEN === secMode) {
        ...
        this.WIFIAUTHMODE_WPA2_PSK === secMode ||
        this.WIFIAUTHMODE_WPA_WPA2_PSK === secMode
      ) {
        $("#" + pwdIDPreffix + "wpa_key_div").show();
      } else if (
        this.WIFIAUTHMODE_802 === secMode ||
        this.WIFIAUTHMODE_WPA_ENTER === secMode ||
        this.WIFIAUTHMODE_WPA_WPA2_ENTER === secMode
      ) {
        $("#" + pwdIDPreffix + "radius_div").show();
      }
    }
  ...
  getWifiKey: function (getKeyDataCallBack, showKeyDataCallBack) {
    ...
    this.postUserPwd(userpwddata, function decryptWifiKey(pwdret) {
      ...
      var hashData = scram.signature(
        CryptoJS.enc.Hex.parse(wifiEncrypted),
        CryptoJS.enc.Hex.parse(hmacKey)
      );
      ...
      var encrypted = CryptoJS.enc.Base64.stringify(
        CryptoJS.enc.Hex.parse(wifiEncrypted)
      );
      var decryptedData = EMUI.wifiObjCtrl.getDAesString(
        encrypted,
        aesKey,
        aesIV
      );
      decryptedData = decryptedData.substring(
        decryptedData.indexOf("<response>")
      );
      var xmls;
      if (
        typeof decryptedData === "string" ||
        typeof decryptedData === "number"
      ) {
  ...
  getKeyData: function (keydata, wifibasic) {
    if (keydata && keydata.type && keydata.type === "response") {
      ...
      $(wifibasic).each(function (i) {
        ...
        if (
          typeof wifibasic[i].WifiRadiusKey !== "undefined" &&
          typeof secretKeyArray[i].WifiRadiusKey !== "undefined"
        ) {
  ...
  validateSsid: function (id, ssidName, supportchinese, supportspecailchar) {
    ...
    for (var i = 0; i < ssidName.length; i++) {
      ...
      if (this.checkInputCharNew(c) || c === "-" || c === "." || c === " ") {
        ...
        c.charCodeAt() <= this.MACRO_SUPPORT_CHAR_MAX &&
        c.charCodeAt() >= this.MACRO_SUPPORT_CHAR_MIN
      ) {
  ...
  isSingleChip: function (featureswitch) {
    ...
    if (!featureswitch.wifi_dbdc_enable || !featureswitch.isdoublechip) {
      return false;
    }
    if (
      featureswitch.wifi_dbdc_enable === "0" &&
  ...
  isDbdcChip: function (featureswitch) {
    ...
    if (
      featureswitch.wifi_dbdc_enable === "1" &&
      featureswitch.isdoublechip === "0"
    ) {
      return true;
    }
  ...
  isOffloadOpen: function (handover) {
    if (
      GLOBAL.modules.wifioffload_enabled &&
  ...
  isGuestwifiOpen: function (featureswitch, wifibasic) {
    ...
    if (guestwifiindex !== -1 && wifibasic[guestwifiindex].WifiEnable === "1") {
      return true;
    }
    return false;
  },
  ...
  is5gRestirct: function (featureswitch, monitorstatus, pinstatus) {
    if (
      !featureswitch.wifi_dfs_enable ||
      featureswitch.wifi_dfs_enable === "0"
    ) {
      return false;
    }
    if (
      monitorstatus.SimStatus === "255" ||
  ...
  getSwitchStatus: function (radioindex, wififeature, wifistatus, wifibasic) {
    ...
    if (this.isDbdcChip(wififeature)) {
      ...
      return wifibasic[this.getMainSsidIndex(radioindex, wififeature)]
        .WifiEnable;
    }
    if (this.isDoubleChip(wififeature)) {
  ...
  getBasicDataFromPage: function (idprefix, basicdata, sourcedata) {
    basicdata.WifiSsid = this.wifiEncode(
      $.trim($("#" + idprefix + "_ssid_input").val())
    );
    basicdata.WifiAuthmode = $("#" + idprefix + "_auth_select").attr("value");
    basicdata.WifiBasicencryptionmodes = this.getBaicencryptionmode(
      basicdata.WifiAuthmode
    );
    basicdata.WifiWpaencryptionmodes = this.getWpaencryptionmode(
      basicdata.WifiAuthmode
    );
    basicdata.WifiWpapsk = null;
    basicdata.MixWifiWpapsk = null;
    if (
      basicdata.WifiAuthmode === this.WIFIAUTHMODE_WPA2_PSK ||
      basicdata.WifiAuthmode === this.WIFIAUTHMODE_WPA_WPA2_PSK
    ) {
      basicdata.WifiWpapsk = this.encryptWifiKey(
        idprefix + "_wpa_key",
        sourcedata.WifiWpapsk
      );
    ...
    if (basicdata.WifiAuthmode === this.WIFIAUTHMODE_AUTO) {
      basicdata.WifiWepKey1 = this.encryptWifiKey(
        idprefix + "_wep_key1",
        sourcedata.WifiWepKey1
      );
      basicdata.WifiWepKey2 = this.encryptWifiKey(
        idprefix + "_wep_key2",
        sourcedata.WifiWepKey2
      );
      basicdata.WifiWepKey3 = this.encryptWifiKey(
        idprefix + "_wep_key3",
        sourcedata.WifiWepKey3
      );
      basicdata.WifiWepKey4 = this.encryptWifiKey(
        idprefix + "_wep_key4",
        sourcedata.WifiWepKey4
      );
      basicdata.WifiWepKeyIndex = $("#" + idprefix + "_key_select").attr(
        "value"
      );
    }
    ...
    if (
      basicdata.WifiAuthmode === this.WIFIAUTHMODE_802 ||
      basicdata.WifiAuthmode === this.WIFIAUTHMODE_WPA_ENTER ||
      basicdata.WifiAuthmode === this.WIFIAUTHMODE_WPA_WPA2_ENTER
    ) {
      basicdata.WifiRadiusAddress = $.trim(
        $("#" + idprefix + "_radius_address").val()
      );
      basicdata.WifiRadiusPort = $.trim(
        $("#" + idprefix + "_radius_port").val()
      );
      basicdata.WifiRadiusKey = this.encryptWifiKey(
        idprefix + "_radius_key",
        sourcedata.WifiRadiusKey
      );
    }
  },
  ...
  isSupportEnterprise: function (wififeature) {
    if (wififeature.radius_enable && wififeature.radius_enable === "1") {
      return true;
    }
    return false;
  },
  ...
  isChangeToWep: function (basicdata, idprefix) {
    if (
      basicdata.WifiAuthmode !== this.WIFIAUTHMODE_AUTO &&
      $("#" + idprefix + "_auth_select").attr("value") ===
        this.WIFIAUTHMODE_AUTO
    ) {
      return true;
    }
    return false;
  ...
  loadWifiConfigureXml: function () {
    ...
    function getWifiConfigure(ret) {
      ...
      if (
        typeof GLOBAL.wifiConfigure.assocnumchangenoshowtips === "undefined"
      ) {
